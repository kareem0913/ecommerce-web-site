...
(function ($) {
                ...
                success : (res) => {
                    ...
                    $('.user-dropdown').html(`
                        <button style='height:0' class='data-user' type='button' data-toggle="modal" data-target='#userModal'>
                            <img class='imageuser' style='width:45px;border-radius:50%' src='${dataUser.image == '' ? 'images/icons/icon-header-01.png' : 'admin/server/api/users/images/'+dataUser.image}' class='header-icon1 mb-3' alt='ICON'>
                        </button>
                        `)
                },
                complete : () => {
                    $('.data-user').on('click', function(){
                        ...
                        $('.user-data-modalbody').html(`
                                <img style='width:200px;border-radius:50%' src='${dataUser.image == '' ? 'images/icons/icon-header-01.png' : 'admin/server/api/users/images/'+dataUser.image}' class='header-icon1 mb-3' alt='ICON'>
                        ...
                        $('.user-data-modalbody').html(`
                            <form>
                                <div class='mb-4 user-image'>
                                    <img style='width:200px;border-radius:50%' src='${dataUser.image == '' ? 'images/icons/icon-header-01.png' : 'admin/server/api/users/images/'+dataUser.image}' class='header-icon1 mb-3' alt='ICON'>
                                    ${dataUser.image != '' ? `<span class='times'><i class='fa fa-times'></i></span>` : ''}
                                </div>
                                ...
                                <div class='form-group mb-3'>
                                    ...
                                    <input name='username' style='background-color:#e7e7e7' type='text' class='form-control' id='exampleInputName' placeholder='Enter name' value=${dataUser.username}>
                                    <div class="err"></div>
                                </div>
                                <div class='form-group mb-3'>
                                    <label for='exampleInputEmail'>Email</label>
                                    <input name = 'email' style='background-color:#e7e7e7' type='text' class='form-control' id='exampleInputEmail' placeholder='Email' value='${dataUser.email}'>
                                    <div class="err"></div>
                                </div>
                                <div class='form-group mb-3'>
                                ...
                                <div class='form-group mb-3'>
                                    ...
                                    <input name = 'phone' style='background-color:#e7e7e7' type='text' class='form-control' id='exampleInputPhone' placeholder='Phone' value='${dataUser.phone}'>
                                    <div class="err"></div>
                                </div>
                                <div class='form-group mb-3'>
                                    <label for='exampleInputaddress'>address</label>
                                    <input name = 'address' style='background-color:#e7e7e7' type='text' class='form-control' id='exampleInputaddress' placeholder='address' value='${dataUser.address}'>
                                    <div class="err"></div>
                                </div>
                            ...
                            $('.user-image').click(function(){
                                ...
                                $.ajax({
                                    ...
                                    data : {id, image},
                                    success : (res) => {
                                        $('.imageuser').attr('src','images/icons/icon-header-01.png');
                                        $('.user-image').html(`<img style='width:200px;border-radius:50%' src='images/icons/icon-header-01.png' class='header-icon1' alt='ICON'>`);
                            ...
                            $('form').on('submit', function(event) {
                                ...
                                if (obj.validateEdit() && obj.Editpassword()) {
                                    ...
                                    if (obj.err.password == 'your password has not changing optional') {
                                        ...
                                        success : (res) => {
                                            ...
                                            if (res.message == 'update success') {
                                                setTimeout(() => {
                                                    ...
                                                    $('.imageuser').attr('src', `${dataUser.image == '' ? 'images/icons/icon-header-01.png' : 'admin/server/api/users/images/'+dataUser.image}`);

                                                }, 300);
                                                dataUser = res.user;   // refresh datauser;
                                            }else{
                                                const errEmail = $('.user-data-modalbody form input[name=email]').siblings('.err');
                                                $(errEmail).html(`<small style=color:red;font-size:15px>${res.message}</small>`);
                                            }
                                        },
                                        error : (err) => {
                                            console.log(err);
                                    ...
                                    elm.forEach((item, i) => {
                                        ...
                                        if (name in obj.err) {
                                            $(item).siblings('.err').html(`<small style=color:red;font-size:15px>${error[name]}</small>`);
                                        }else{
                                            $(item).siblings('.err').html();
                                        }
                                    });
    ...
    $(document).ready(function(){
            ...
            $('.js-show-header-dropdown').click(function(){
                $.ajax({
                    ...
                    success : (res) => {
                        ...
                        let element = '';
                        res.forEach((item, i) => {
                            'numpro' in item ? total = total + [item.price - item.sale ] * item.numpro : total = item.price - item.sale + total;
                            price_cart[item.id] = 'numpro' in item ? [item.price - item.sale ] * item.numpro : item.price - item.sale;    // make Object for cart;
                            let image = item.img.split(',');
                            element += `<li class='header-cart-item'>
                                <div data_id='${item.id}' class='header-cart-item-img'>
                                    <img src='admin/server/api/products/images/${image[0]}' alt='IMG'>
                                </div>

                                <div class='header-cart-item-txt'>
                                    <a href='product-detail.php?id=${item.id}' class='header-cart-item-name'>
                                        ${item.name}
                                    </a>

                                    <span class='header-cart-item-info'>
                                        ${'numpro' in item ? item.numpro : 1}x$${item.price - item.sale }
                                    </span>
                                </div>
                            </li>`
                        });
                        ...
                        $('.header-cart-item-img').click(function(){
                            ...
                            $.ajax({
                                ...
                                success : (res) => {
                                    ...
                                    $(this).parent().remove();
                                    $('.header-cart-total-sh').text(price_cart.total-price_cart[id]);
        ...
        // footer category 
    $(document).ready(() => {
        const url = "http://localhost/ecommerce/admin/server/api/products/category.php"
        $.get(url, res => {
            res.forEach(element => {
